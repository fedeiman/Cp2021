...
inline __m256i random_vector(rMTRand r) {
    return _mm256_set_ps(genRand(&r),genRand(&r),genRand(&r),genRand(&r),genRand(&r),genRand(&r),genRand(&r),genRand(&r));
}
    ...
    */
    ...
    while(photon_count < PHOTONS){
       ...
       __m256 heat_res_squared = _mm256_mul_ps(heat_res, heat_res);
        ...
        } while (_mm_movemask_ps(randMask) != 0xFF );
        ...
        */
        ...
        x = _mm256_blendv_ps(x, zeros, randMask );
        y = _mm256_blendv_ps(y, zeros, randMask );
        z = _mm256_blendv_ps(z, zeros, randMask );
        u = _mm256_blendv_ps(u, zeros, randMask );
        v = _mm256_blendv_ps(v, zeros, randMask );
        w = _mm256_blendv_ps(w, ones, randMask );
}
