    ...
    if(gtid <= PHOTONS){
        ...
        for (;;) {
            ...
            if(prevShell == shell){
              ...
              if(prevShell >= 0){
                atomicAdd(&local_heat[prevShell], cachedW);
                atomicAdd(&local_heat2[prevShell], cachedWS); 
              }
              cachedW = (1.0f - albedo) * weight;
              cachedWS = (1.0f - albedo) * (1.0f - albedo) * weight * weight;
              prevShell = shell;
     ...
     __syncthreads();
    // the first 101 threads make the partial sums
    for (int i = threadIdx.x; i < SHELLS; i += blockDim.x){
      atomicAdd(&globHeat.heat[i], local_heat[i]);
      atomicAdd(&globHeat.heat2[i], local_heat2[i]);
    }
}
